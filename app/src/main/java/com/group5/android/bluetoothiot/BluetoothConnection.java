...
/**
 * Created by TrinhLeChiNguyen on 09/05/2017.
 */
...
public class BluetoothConnection {
        ...
        public void block() {
            ...
            while (done == false){
                try {
                    ...
                    //check connection again or keep waiting
                }
            }
        }
    ...
    private static class ConnectThread extends Thread{
        ...
        public ConnectThread(BluetoothDevice device, BluetoothAdapter adapter, ConnectionFuture connectionFuture){
            this.device = device;
        ...
        public void run(){
            ...
            while (true){
                try {
                    ...
                    if (readHandler != null){
                        readHandler.obtainMessage(MESSAGE_READ, numBytes, -1, buffer).sendToTarget();
                    }
                } catch (IOException e) {
                    e.printStackTrace();
                }
    ...
    public BluetoothConnection (BluetoothDevice device, BluetoothSocket socket, Handler readHandler)
            throws IOException {
        this.device = device;
        this.socket = socket;
